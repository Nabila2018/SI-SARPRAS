...
<div class="max-w-7xl mx-auto pb-12">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            @if(auth()->user()->role->nama_role === 'Petugas UPTD' && $laporan->id_pelapor === auth()->id() && $laporan->status_laporan === 'Menunggu' && is_null($laporan->id_evaluator) && is_null($laporan->kategori_kerusakan))
                <a href="{{ route('laporan.edit', $laporan->id_laporan) }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#114F72] hover:bg-[#0e405d] text-white text-xs font-semibold rounded-xl shadow-sm transition">
    ...
    @php
        ...
        $evaluasiApproved = !is_null($laporan->kategori_kerusakan) && (
            !is_null($laporan->tanggal_verifikasi_evaluasi) ||
            $laporan->status_laporan === 'Disetujui' ||
            !is_null($laporan->id_rab)
        ) && $isReportActive;
        $evaluasiDone = $evaluasiApproved;
