                        ...
                        <tr class="border-b border-gray-100">
                            ...
                            <td class="py-3 px-2 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button"
                                            onclick="openKabidPreviewBuktiModal('{{ asset('storage/' . $bukti->file_bukti) }}', '{{ basename($bukti->file_bukti) }}')"
...
<!-- Modal Pratinjau Bukti Pembelian Kabid -->
<div id="kabidPreviewBuktiModal"
     class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 px-4"
     onclick="if(event.target === this) closeKabidPreviewBuktiModal()">
    <div class="w-full max-w-4xl rounded-2xl bg-white p-6 shadow-2xl space-y-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between border-b pb-3">
            <div>
                ...
                <p id="kabidPreviewBuktiSubtitle" class="text-xs text-gray-500"></p>
            </div>
            <button type="button" onclick="closeKabidPreviewBuktiModal()" class="text-gray-400 hover:text-gray-600">
        ...
        <div id="kabidPreviewBuktiBody" class="flex items-center justify-center min-h-[50vh] max-h-[75vh] overflow-auto">
            <!-- Rendered by JS -->
        </div>
    </div>
</div>
...
<script>
    function openKabidPreviewBuktiModal(url, filename) {
        document.getElementById('kabidPreviewBuktiSubtitle').textContent = filename || '';
        const body = document.getElementById('kabidPreviewBuktiBody');
        ...
        const modal = document.getElementById('kabidPreviewBuktiModal');
        modal.classList.remove('hidden');
        modal.classList.add('flex');
        document.body.style.overflow = 'hidden';
    }

    function closeKabidPreviewBuktiModal() {
        const modal = document.getElementById('kabidPreviewBuktiModal');
        if (!modal) return;
